...
export class AuthGuardGuard implements CanActivate, CanActivateChild {
  ...
  canActivate(
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean |
     UrlTree {
    return this.authSrv.isLoggedIn$.pipe(take(1), map((variabile)=>{  //se il dato per decidere il return deriva da un observagble allora useremo .pipe(map(()=>{return true})) per generare un observable con return boolean | urlTree e restituiremo quello
      if (variabile){
        return true;
      }
